...
async def start_cmd(client: Client, message: Message):
    ...
    caption = (
        ...
        "I can help you with:\n"
        "• Unzipping 20+ archive formats (ZIP/RAR/7Z/TAR… + passwords)\n"
        "• Extracting audio from any video\n"
        "• Handling direct download links, Google Drive links, m3u8 streams\n"
        "• Listing extracted files & sending single/all\n\n"
        "Use /help to see full usage guide."
    )
...
async def help_cmd(client: Client, message: Message):
    text = (
        ...
        "<b>1. Basic Commands</b>\n"
        "• <code>/start</code> – Show welcome screen & quick intro.\n"
        "• <code>/help</code> – Show this help message.\n"
        "• <code>/settings</code> – View basic settings (auto-delete, language etc.).\n"
        "• <code>/cancel</code> – Mark your current task as cancelled (next step will stop).\n\n"
        "<b>2. Archive Handling (ZIP / RAR / 7Z / TAR / GZ...)</b>\n"
        "1) Send or forward any archive file to the bot (DM, group or channel).\n"
        "2) Bot will show buttons under the file:\n"
        "   • <b>📦 Unzip</b> – Extract without password.\n"
        "   • <b>🔐 With Password</b> – Bot asks for password, then extracts.\n"
        "3) After extraction you get:\n"
        "   • Summary of file types (videos, PDFs, APKs, TXT, m3u8…)\n"
        "   • Inline list of files (first few) – tap to receive that single file.\n"
        "   • <b>Send ALL</b> button – sends every extracted file back to you.\n"
        "   • Videos are sent as <b>playable videos</b> with a random emoji in the title.\n\n"
        "<b>3. Videos & Audio</b>\n"
        "• Send any video → bot shows <b>🎧 Extract Audio</b> button.\n"
        "  - It downloads the video, extracts audio via ffmpeg and returns it as a file.\n"
        "• Extracted or downloaded videos are sent as real Telegram videos, not documents,\n"
        "  with an auto-generated thumbnail taken from inside the video.\n\n"
        "<b>4. Links / TXT Parser (Direct, m3u8, Google Drive)</b>\n"
        "1) Send a message or .txt file in DM containing links.\n"
        "2) Bot detects URLs and offers:\n"
        "   • <b>Download all videos/files</b>\n"
        "   • <b>Cleaned TXT only</b> (just list of URLs)\n"
        "   • <b>Skip</b>\n"
        "3) On <b>Download all</b>:\n"
        "   • Direct file links (e.g. mp4/mkv/zip/apk/xapk/audio) are downloaded with\n"
        "     progress bar (speed + ETA) and then sent back.\n"
        "   • Google Drive links are converted to direct-download where possible and\n"
        "     downloaded similarly. (Very large files or protected links may fail.)\n"
        "   • m3u8 links:\n"
        "       - Bot analyses the playlist; if multiple qualities are present, you get\n"
        "         buttons like 360p / 720p etc.\n"
        "       - You choose a quality → ffmpeg downloads the stream → bot sends mp4.\n"
        "       - If only one variant is available, you’ll see a single “Auto” quality.\n\n"
        "<b>5. Groups & Channels</b>\n"
        "• When added to groups/channels (with proper rights), bot can:\n"
        "  - Respond to archives/videos with unzip/extract buttons.\n"
        "  - (Experimental) Auto-react to posts with random emoji (if Telegram allows\n"
        "    reactions in that chat).\n\n"
        "<b>6. Admin Commands</b> (Owners only)\n"
        "• <code>/status</code> – Show user counts and disk usage.\n"
        "• <code>/broadcast</code> – Reply to a message and broadcast it to all users.\n"
        "• <code>/ban &lt;user_id&gt;</code> / reply – Ban a user.\n"
        "• <code>/unban &lt;user_id&gt;</code> / reply – Unban a user.\n"
        "• <code>/clean</code> – Trigger manual cleanup of temporary files.\n\n"
        "<i>Note:</i> Very large Google Drive files or special links that require\n"
        "web-confirmation might fail – that’s a Google limitation, not Telegram’s.\n"
    )
...
async def settings_cmd(client: Client, message: Message):
    text = (
        "<b>Settings</b>\n\n"
        "Current defaults:\n"
        "• Auto delete temp files after: <b>30 minutes</b>\n"
        "• Default extract mode: <b>full archive</b>\n"
        "• Language: <b>English</b>\n\n"
        "Advanced per-user settings (auto delete, preferred output, language, etc.)\n"
        "will be configurable from here in future updates."
    )
...
async def run_unzip_task(client: Client, msg: Message, password: Optional[str]):
    ...
    async with lock:
        ...
        stats = result["stats"]
        files = sorted(result["files"], key=lambda p: p.lower())  # series order
    ...
    for rel in files:
        ...
        try:
            if is_video_path(rel):
                # video as playable with random emoji title + thumbnail
                name = Path(rel).name
                caption = f"{random_emoji()} {name}"
...
async def handle_links_download_all(
    ...
    for url in candidate_direct:
        ...
        base = base_raw.rsplit("/", 1)[-1] or f"file_{uuid.uuid4().hex}"
        ...
        try:
            ...
            if is_video_path(base):
                caption = f"{random_emoji()} {base}"
                ...
                await client.send_video(chat_id, dest_path, caption=caption, thumb=thumb_arg)
            else:
                await client.send_document(chat_id, dest_path, caption=base)
            ok += 1
        except Exception:
            fail += 1
        await asyncio.sleep(0.5)
    ...
    for url in gdrive_links:
        ...
        base = base_raw.rsplit("/", 1)[-1] or f"gdrive_{uuid.uuid4().hex}"
        dest_path = str(temp_root / base)
        try:
            status = await client.send_message(
                chat_id,
...
async def offer_m3u8_quality_menu(
    ...
    # Hamesha menu show karo; agar sirf 1 variant hai to 'Auto'
    if not variants:
...
async def handle_m3u8_quality_choice(
    ...
    caption = f"{random_emoji()} {base_name} [{name}]"
    thumb_path = dest_path + ".jpg"
    try:
        await generate_thumbnail(dest_path, thumb_path)
        thumb_arg = thumb_path
