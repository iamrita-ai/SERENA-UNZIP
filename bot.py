...
# video ext set (for send_video) and emoji
...
async def on_file(client: Client, message: Message):
    ...
    try:
        if message.document:
            ...
            cap = (
                f"From user {user_id} (@{message.from_user.username or 'N/A'})\n"
...
async def run_unzip_task(client: Client, msg: Message, password: Optional[str]):
    ...
    async with lock:
        ...
        stats = result["stats"]
        # important: sort files for stable series order
    ...
    for rel in files:
        ...
        try:
            if is_video_path(rel):
                # video as playable with random emoji title
                name = Path(rel).name
    ...
    for url in candidate_direct:
        ...
        try:
            ...
            if is_video_path(base):
                ...
                await client.send_video(chat_id, dest_path, caption=caption)
            else:
...
async def offer_m3u8_quality_menu(
    ...
    if len(variants) == 1:
        ...
        try:
            ...
            caption = f"{random_emoji()} {base_name} [{v['name']}]"
            await client.send_video(
                chat_id,
                dest_path,
                caption=caption,
