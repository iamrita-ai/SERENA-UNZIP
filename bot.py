...
def is_video_file(name: str) -> bool:
    n = name.lower()
    return any(n.endswith(ext) for ext in (".mp4", ".mkv", ".mov", ".avi", ".webm", ".ts"))
...
@app.on_message(
    (filters.document | filters.video) & (filters.private | filters.group | filters.channel)
)
...
async def handle_links_download_all(
    ...
    if not direct_links and not m3u8_links and not gdrive_links:
        await cq.message.edit_text(
    ...
    await cq.message.edit_text(
        f"Direct: {len(direct_links)} | GDrive: {len(gdrive_links)} | m3u8: {len(m3u8_links)}\n"
    ...
    # direct files (mp4/mkv/zip/apk/... original extension)
    ...
    for url in gdrive_links:
        ...
        dest_path = str(temp_root / base)
        try:
            await download_file(direct_url, dest_path)
            await cq.message.chat.send_document(dest_path, caption=base)
            ok += 1
        except Exception:
            fail += 1
    ...
    # m3u8 -> quality selection menus
    for url in m3u8_links:
        await offer_m3u8_quality_menu(cq, user_id, url, temp_root)
...
async def offer_m3u8_quality_menu(
    cq: CallbackQuery, user_id: int, url: str, temp_root: Path
):
    try:
    ...
    if len(variants) == 1:
        # single quality -> auto download
        v = variants[0]
        await cq.message.chat.send_message(
            f"m3u8 stream mila (Auto). Downloading…"
        )
        dest_path = str(temp_root / f"{base_name}.mp4")
        try:
            await download_m3u8_stream(v["url"], dest_path)
            await cq.message.chat.send_video(
                dest_path, caption=f"{base_name} [{v['name']}]"
            )
        except Exception as e:
            await cq.message.chat.send_message(
                f"m3u8 download fail:\n<code>{e}</code>"
            )
        return

    # multiple qualities -> ask user
    task_id = uuid.uuid4().hex
    M3U8_TASKS[task_id] = {
        "user_id": user_id,
        "url": url,
...
@app.on_message((filters.chat_type.groups | filters.chat_type.channel) & ~filters.service)
async def auto_react_handler(client: Client, message: Message):
    # bot / empty / service messages skip
    ...
    try:
        ...
        # permission / feature not available -> ignore
        pass
